...
function update_time_steps(){
    time_elapsed = Date.now() - start_time;
    delta_time = time_elapsed - prev_time;
    prev_time = time_elapsed;
    
    if(time_elapsed > fps_refresh_timer){
      fps = (1000 / delta_time).toFixed(2);
      fps_refresh_timer += 500;
    }
}
...
function drawImageFromSpriteSheetWithRotation(img, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight, ang, antiAli){
    ...
    if(antiAli) canvasContext.imageSmoothingEnabled = true; 
    canvasContext.drawImage(img, sx, sy, sWidth, sHeight, -dWidth/2, -dHeight/2, dWidth, dHeight);
    canvasContext.restore();
}

function distance_of_two_points(x1, y1, x2, y2){
    return Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2))
}

function intersectLineWithXYPlane(v1, v2) {
  const z1 = v1[2], z2 = v2[2];

  // If both points are above or below the plane, no intersection
  if ((z1 > 0 && z2 > 0) || (z1 < 0 && z2 < 0)) {
      return null;
  }

  // Compute the interpolation factor t where the line crosses z = 0
  const t = -z1 / (z2 - z1);

  // Compute intersection point
  const x = v1[0] + t * (v2[0] - v1[0]);
  const y = v1[1] + t * (v2[1] - v1[1]);
  const z = 0.000001; // Since it's the XY plane
